...
public class AdminQueries extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAdapter = new QueriesAdapter(this, "1", mQueryList, new QueriesAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(ItemQueriesPojo.QueryList response, int position) {
                Intent intent = new Intent(AdminQueries.this, UserItemWiseQueries.class);
    ...
    private void getUserQueryData(ItemQueriesPojo.QueryList data, int position) {
        try {
            Call<ItemQueriesPojo> call = apiService.getQueries("", data.getUserId(), "1", "");
    ...
    private void getQueryData() {
        try {
            ...
            Call<ItemQueriesPojo> call = apiService.getQueries("", "", "admin", "");
            call.enqueue(new Callback<ItemQueriesPojo>() {
                @Override
                public void onResponse(Call<ItemQueriesPojo> call, Response<ItemQueriesPojo> response) {
                    ItemQueriesPojo mPojo = response.body();
    ...
    private void chatDialog(List<ItemQueriesPojo.QueryList> response, int position) {
        ...
        ChatAdapter mAdapter = new ChatAdapter(AdminQueries.this, response);
